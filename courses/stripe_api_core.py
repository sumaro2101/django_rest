...
class StripeItem:
    ...
    def _create_price_product(self,
                              product,
                              created_product: stripe.Product,
                              ) -> stripe.Price:
        """Создание данных о цене
        """
        product_amount = product.amount    
        created_price_product = stripe.Price.create(
            unit_amount_decimal=product_amount,
            currency='rub',
            product=created_product.id,
            )
        return created_price_product

    def _check_product_in_stripe(self,
                                 product_name,
                                 ) -> Union[stripe.Price, None]:
        """Проверка нахождения продукта в Stripe
        """
        get_product = stripe.Product.search(
            query=f'name~"{product_name}"',
            )
        if get_product:
            get_price = stripe.Price.search(
                query=f'product:"{get_product.data[0].id}"',
                )
            return get_price
    ...
    def _set_object_stripe(self, product) -> stripe.Price:
        ...
        if not stripe_product:    
            ...
            stripe_product_price = self._create_price_product(
                product,
                stripe_product,
                )
        else:
...
class SessionLinkPayment:
    ...
    def __init__(self,
        ...
        if not isinstance(product, Union[Course, Lesson]):
            raise TypeError(
                f'{product}, должен представлять собой Курс или Урок',
                )
        self.product = product
        self.stripe_item = StripeItem(product)
        if not isinstance(request, Request):
            raise TypeError(
                f'{request} должен быть классом '
                'rest_framework.request.Request',
                )
        self.request = request

    def _create_link_payments(self,
                              created_price_product: stripe.Price,
                              ) -> stripe.PaymentLink:
        """Создание ссылки для оплаты
        """
        id_price_product = created_price_product.id
        created_product_link_payment = stripe.PaymentLink.create(
            line_items=[
                {'price': id_price_product,
                 'quantity': 1,
                 },
                ],
            )
    ...
    def _create_session_payments(self,
                                 created_price_product: stripe.Price,
        ...
        created_sessions_payments = stripe.checkout.Session.create(
            line_items=[
                {'price': id_price_product,
                 'quantity': 1,
                 },

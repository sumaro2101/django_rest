...
class TestCourseAPI(APITestCase):
    ...
    def test_view_course(self):
        """Тест вывода курса
        """        
        course = Course.objects.create(owner=self.user, course_name='course', description='description_of_the_course')
        url = self.course_url + str(course.pk) + '/'
        response = self.client.get(url)
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'pk': course.pk,
                                         'course_name': 'course',
                                         'course_preview': None,
    ...
    def test_view_list_courses(self):
        ...
        """        
        Course.objects.create(owner=self.user, course_name='course_first', description='description_of_the_course_first')
        Course.objects.create(owner=self.user, course_name='course_second', description='description_of_the_course_second')
        url = self.course_url
    ...
    def test_create_course(self):
        ...
        self.assertEqual(Course.objects.get().description, 'description_of_the_course')
    ...
    def test_update_course(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Course.objects.get().description, 'update_description')
    ...
    def test_delete_course(self):
        ...
        self.assertFalse(Course.objects.filter(course_name='course').exists())
...
class TestLessonAPI(APITestCase):
    ...
    def setUp(self):
        self.user = get_user_model().objects.create(username='test', email='test@gmail.com', password='testroot')
        self.client.force_authenticate(user=self.user)
        self.course = Course.objects.create(owner=self.user, course_name='course', description='description_of_the_course')
        
    def test_view_lesson(self):
        """Тест вывода урока
        """        
        lesson = Lesson.objects.create(owner=self.user,
                                       course=self.course,
                                       lesson_name='lesson',
                                       description='description_of_the_lesson',
                                       video_link='https://www.youtube.com/some-video/',
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'pk': lesson.pk,
    ...
    def test_view_list_lessons(self):
        ...
        """        
        Lesson.objects.create(owner=self.user,
                              course=self.course,
    ...
    def test_create_lesson(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Lesson.objects.filter(lesson_name='lesson').exists())
    ...
    def test_create_common_link_video_lesson(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Lesson.objects.filter(video_link='path/to/some/video').exists())
        self.assertEqual(Lesson.objects.count(), 1)
    ...
    def test_update_lesson(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(Lesson.objects.filter(description='update_description').exists())
        self.assertFalse(Lesson.objects.filter(description='description_of_the_lesson').exists())
...
class TestSubscribeAPI(APITestCase):
    ...
    def test_set_subscribe(self):
        ...
        """        
        url_subscribe = reverse('courses:subscribe', kwargs={'pk': self.course.pk})
